...
def train_agent(results_dir, cfg):
    ...
    csv_header = [
    'step', 'avg_reward', 'std_reward', 'avg_ep_length', 
    ...
    with tqdm(total=max_timesteps, desc="Total Training Progress") as pbar:
        while (training_iters < max_timesteps):
            ...
            loss_metric = agent.train(data_sampler, iterations=iterations, batch_size=cfg.batch_size, progress_bar=pbar)
            training_iters += iterations
            curr_epoch = int(training_iters // int(cfg.num_steps_per_epoch))

            # Evaluation
            eval_env = gym.make(gym_env_name)
            eval_res, eval_res_std, avg_ep_length = eval_policy(agent, eval_env, cfg.seed, eval_episodes=cfg.eval_episodes)
            
            # Logging
            log_row = [training_iters, eval_res, eval_res_std, avg_ep_length,
                    np.mean(loss_metric['bc_loss']), np.mean(loss_metric['ql_loss']), 
                    np.mean(loss_metric['actor_loss']), np.mean(loss_metric['critic_loss']),
                    np.mean(loss_metric['avg_q_batch']), np.mean(loss_metric['avg_q_policy']),
                    np.mean(loss_metric['actor_gn']), np.mean(loss_metric['critic_gn'])]
            ...
            if cfg.save_best_model:
                agent.save_model(results_dir, id=None) # Can change to curr_epoch if you want all
