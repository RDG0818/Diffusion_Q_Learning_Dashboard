...
class StudentPolicy(nn.Module):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(256, 256), nn.ReLU(),  # third hidden
...
def load_agent_params(model_dir: str, params_path: str):
    ...
    agent = Diffusion_QL(
        ...
        device=device, discount=params.get('discount', 0.99), tau=params.get('tau', 0.005),
        max_q_backup=params.get('max_q_backup', False), beta_schedule=params.get('beta_schedule','linear'),
        n_timesteps=params.get('T', 5), eta=params.get('eta',1.0), lr=params.get('lr',3e-4),
        lr_decay=params.get('lr_decay',False), lr_maxt=params.get('num_epochs',100), grad_norm=params.get('gn',5.0)
    )
...
def distill(cfg: DictConfig) -> None:
    ...
    teacher, device, teacher_params = load_agent_params(cfg.teacher_model_dir, teacher_p)
    ...
    optimizer = torch.optim.Adam(student.parameters(), lr=cfg.distill_lr, weight_decay=cfg.weight_decay)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=cfg.distill_epochs)
    ...
    for itr in range(1, cfg.dagger_iters+1):
        ...
        for _ in range(cfg.dagger_rollouts):
            ...
            while not done:
                ...
                # query teacher
                with torch.no_grad():
                    t_a = teacher.actor.sample(torch.from_numpy(s).float().unsqueeze(0).to(device))
                dagger_actions.append(t_a.cpu().numpy()[0])
                s, _, terminated, truncated, _ = env.step(a)
                done = terminated or truncated
        # Convert buffer to tensors
        buf_states = torch.tensor(np.array(dagger_states), dtype=torch.float32, device=device)
        buf_actions = torch.tensor(np.array(dagger_actions), dtype=torch.float32, device=device)
        ...
        for ep in tqdm(range(cfg.dagger_epochs), desc="DAgger Train", unit="ep"):
            ...
            s2 = buf_states[idx]; a2 = buf_actions[idx]
    ...
    t_r, t_l, t_t = eval_policy(teacher, eval_env, seed=cfg.seed, eval_episodes=cfg.eval_episodes)
    s_r, s_l, s_t = eval_policy(student, eval_env, seed=cfg.seed, eval_episodes=cfg.eval_episodes)

    # Save metrics
    rows=[]
    for i,(r,l,tm) in enumerate(zip(t_r,t_l,t_t)): rows.append({'model':'teacher','episode':i+1,'reward':r,'length':l,'time':tm})
    for i,(r,l,tm) in enumerate(zip(s_r,s_l,s_t)): rows.append({'model':'student','episode':i+1,'reward':r,'length':l,'time':tm})
    pd.DataFrame(rows).to_csv(cfg.student_model_save_path.replace('.pt','_dagger_eval.csv'), index=False)
